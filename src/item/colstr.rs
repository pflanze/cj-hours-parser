...
impl<'a> Colstr<'a> {
    ...
    pub fn parse_digits(&self, mindigits: u32, maxdigits: u32)
        ...
        self._parse_digits(mindigits, maxdigits).or_else(
            |e| match e {
                ParseDigitsError::NotEnoughDigits => 
                    bail!("column {}: not enough digits, min required {}",
                          self.column(), mindigits),
                ParseDigitsError::TooManyDigits =>
                    bail!("column {}: too many digits, max allowed {}",
                          self.column(), maxdigits),
                ParseDigitsError::NumberOverflow =>
